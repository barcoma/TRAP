...
export default {
  ...
  methods: {
    ...
    searchPOI: function(event) {
      ...
      var customResults = this.$apollo.queries.customPOI.refetch({term: event.target.value, latitude: this.lat, longitude: this.long});
      var resultPromise = this.$apollo.queries.yelpPOI.refetch({ term: event.target.value, latitude: this.lat, longitude: this.long, radius: 5000, limit: 15 })
